create table actionhistory(
  userid varchar,
  eventype int, -- 1 is mfa+ , 2 is mfa- , other is other
  timestamp timestamp,
  changenum int,
);
...
create materialized view user_action_mfa as select userid,eventype, timestamp,changenum from actionhistory where eventype in (1,2);
...
create function udf_sum(int, int) returns int as sum using link 'http://localhost:8815';
